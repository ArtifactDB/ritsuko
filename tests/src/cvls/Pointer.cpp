...
TEST(CvlsDefinePointerDatatype, Basic) {
    {
        auto dtype = ritsuko::cvls::define_pointer_datatype<uint64_t, uint64_t>();
        ritsuko::cvls::validate_pointer_datatype(dtype, 64, 64);
    }

    // Works correctly with uneven types.
    {
        auto dtype = ritsuko::cvls::define_pointer_datatype<uint64_t, uint32_t>();
        ritsuko::cvls::validate_pointer_datatype(dtype, 64, 64);
    }

    // Works correctly with smaller, uneven types.
    {
        auto dtype = ritsuko::cvls::define_pointer_datatype<uint8_t, uint32_t>();
        ritsuko::cvls::validate_pointer_datatype(dtype, 32, 32);
    }
}

TEST(CvlsDefinePointerDatatype, Usage) {
    size_t nlen = 10;
    ...
    {
        std::vector<ritsuko::cvls::Pointer<uint32_t, uint64_t> > roundtrip(nlen);
        auto out_type = ritsuko::cvls::define_pointer_datatype<uint32_t, uint64_t>();
    ...
    {
        std::vector<ritsuko::cvls::Pointer<uint64_t, uint16_t> > roundtrip(nlen);
        auto out_type = ritsuko::cvls::define_pointer_datatype<uint64_t, uint16_t>();
...
TEST(CvlsValidatePointerDatatype, Failure) {
    ...
    {
        typedef ritsuko::cvls::Pointer<uint8_t, uint8_t> tmp;
...

    {
        ...
        EXPECT_ANY_THROW({
            try {
                ritsuko::cvls::validate_pointer_datatype(dtype, 32, 64);
            } catch (std::exception& e) {
                EXPECT_THAT(e.what(), ::testing::HasSubstr("exceed"));
                throw;
            }
...

    {
        ...
        EXPECT_ANY_THROW({
            try {
                ritsuko::cvls::validate_pointer_datatype(dtype, 64, 32);
            } catch (std::exception& e) {
                EXPECT_THAT(e.what(), ::testing::HasSubstr("exceed"));
                throw;
            }
