...
TEST(CvlsOpen, Pointers) {
    ...
    auto dhandle = ritsuko::cvls::open_pointers(handle, "foo", 32, 32);
    EXPECT_TRUE(dhandle.getTypeClass() == H5T_COMPOUND);

    EXPECT_ANY_THROW({
        try {
            ritsuko::cvls::open_pointers(handle, "foo", 8, 8);
    ...
    EXPECT_ANY_THROW({
        try {
            ritsuko::cvls::open_pointers(handle, "bar", 8, 8);
        } catch (std::exception& e) {
...
TEST(CvlsOpen, Concatenated) {
    ...
    // Checking that it opens correctly.
    H5::H5File handle(path, H5F_ACC_RDONLY);
    auto dhandle = ritsuko::cvls::open_heap(handle, "foo");
    EXPECT_TRUE(dhandle.getTypeClass() == H5T_INTEGER);

    EXPECT_ANY_THROW({
        try {
            ritsuko::cvls::open_heap(handle, "bar");
        } catch (std::exception& e) {
    ...
    EXPECT_ANY_THROW({
        try {
            ritsuko::cvls::open_heap(handle, "other");
        } catch (std::exception& e) {
    ...
    EXPECT_ANY_THROW({
        try {
            ritsuko::cvls::open_heap(handle, "more");
        } catch (std::exception& e) {
