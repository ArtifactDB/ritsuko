#ifndef RITSUKO_CVLS_OPEN_HPP
#define RITSUKO_CVLS_OPEN_HPP
...
/**
 * @file open.hpp
 * @brief Open HDF5 datasets for a compressed VLS array.
 */
...
/**
 * Open a HDF5 dataset containing pointers into the compressed VLS heap, used to define the individual strings in a compressed VLS array.
 *
 ...
 */
inline H5::DataSet open_pointers(const H5::Group& handle, const char* name, std::size_t offset_precision, std::size_t length_precision) {
    auto dhandle = hdf5::open_dataset(handle, name);
    if (dhandle.getTypeClass() != H5T_COMPOUND) {
        throw std::runtime_error("expected a compound datatype for a compressed VLS pointer dataset at '" + hdf5::get_name(dhandle) + "'");
    }
    try {
        validate_pointer_datatype(dhandle.getCompType(), offset_precision, length_precision);
    } catch (std::exception& e) {
        throw std::runtime_error("incorrect type for a compressed VLS pointer dataset at '" + hdf5::get_name(dhandle) + "; " + std::string(e.what()));
    }
    return dhandle;
}

/**
 * Open a HDF5 dataset containing the compressed VLS heap.
 * This should be a 1-dimensional dataset of unsigned 8-bit integers, representing the concatenation of bytes from all the variable length strings in the compressed VLS array.
 * Ideally, all bytes are referenced by at least one `Pointer` in the associated pointer dataset, though this is not required, e.g., if a heap entry is replaced with a shorter string.
 *
...
inline H5::DataSet open_heap(const H5::Group& handle, const char* name) {
    ...
    if (dhandle.getTypeClass() != H5T_INTEGER) {
        throw std::runtime_error("expected an integer datatype for the compressed VLS heap at '" + hdf5::get_name(dhandle) + "'");
    }
    if (hdf5::exceeds_integer_limit(dhandle.getIntType(), 8, false)) {
        throw std::runtime_error("expected 8-bit unsigned integers for the compressed VLS heap at '" + hdf5::get_name(dhandle) + "'");
    }
    if (dhandle.getSpace().getSimpleExtentNdims() != 1) {
        throw std::runtime_error("expected a 1-dimensional dataset for the compressed VLS heap at '" + hdf5::get_name(dhandle) + "'");
    }
    return dhandle;
}
